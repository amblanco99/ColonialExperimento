...
export function crearAgentesButterfly(containerId) {
  ...
  const estadoLocal = { crimenLocal: null };
  let ultimoEventos = [];
  ...
  function actualizar(eventosFiltrados, crimenTop = "Todos") {
    ...
    if (enSubnivel) {
      ...
      nombresFila = [...d3.rollup(filasBase.filter(d => d.subcrimen), v => v.length, d => d.subcrimen)]
        .sort((a, b) => b[1] - a[1])
        .map(([nombre]) => nombre);
    } else {
      filasBase = eventosFiltrados;
      nombresFila = [...d3.rollup(eventosFiltrados, v => v.length, d => d.crimen)]
        .sort((a, b) => b[1] - a[1])
    ...
    const filasConDatos = nombresFila.filter(f => conteoInst.get(f) > 0 || conteoPobl.get(f) > 0);
    const IH = filasConDatos.length * PASO_FILA;
    const HEIGHT = MARGIN.top + IH + MARGIN.bottom;
    const máximoFila = d3.max(filasConDatos, f => Math.max(conteoInst.get(f), conteoPobl.get(f))) || 1;
    ...
    filasConDatos.forEach(nombreFila => {
      ...
      function dibujarLado(valor, grupo, esDerecha) {
        ...
        const color = PALETA_GRUPO[grupo] || "var(--accent)";
        if (d) {
