...
export async function crearPersonasDashboard() {
  ...
  const datosBase = raw
    ...
    .map((d) => {
      const genero =
        d.Género && d.Género.trim() !== '' && d.Género !== 'null'
          ? d.Género.trim()
          : 'Sin información';
      ...
      const subcrimen =
        ...
        d.Nombre_Sub_Codigo.trim().toUpperCase() !== 'NULL'
          ? d.Nombre_Sub_Codigo.trim()
          : null;
      const año = +d.Año;
      return {
  ...
  function renderFiltrosTop() {
    ...
    labelGenero.textContent =
      'Género (línea de tiempo y sunburst; el butterfly usa sus propios selectores)';
  ...
  function actualizarTodo() {
    ...
    const datosFecha = datosBase.filter(
      (d) => d.década >= estado.decadaDesde! && d.década <= estado.decadaHasta!,
    );
    ...
    const datosFechaCrimen =
      estado.crimen === 'Todos' ? datosFecha : datosFecha.filter((d) => d.crimen === estado.crimen);
    ...
    dibujarDelitosSunburstGenero(
      'delitosGeneroSunburst',
      filasSunburst,
      estado.crimen === 'Todos' ? null : estado.crimen,
    );
  }
