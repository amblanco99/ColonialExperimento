...
export async function crearConteoInteractivo() {
  ...
  const statsPorTipo = new Map(
    TIPOS_OTROS_AGENTES.map(({ label }) => [
      label,
      resumirGrupo(datos.filter((d) => d.Agente === label)),
    ]),
  );
  ...
  dibujarToggle(
    ...
    [
      {
        label: 'Personas',
        color: 'var(--genero-mujer)',
        texto: 'var(--ink)',
        destino: `${import.meta.env.BASE_URL}personas/personas.html`,
        vista: vistaPersonas,
      },
      {
        label: 'Otros agentes',
        color: 'var(--tipo-institucion)',
        texto: '#fff',
        destino: `${import.meta.env.BASE_URL}personas/otrosAgentes.html`,
        vista: vistaAgentes,
      },
    ],
    (opcion: any) => {
  ...
  dibujarCuadrosConDetalle(
    vistaAgentes,
    TIPOS_OTROS_AGENTES.map(({ label, nombre }) => ({
      clave: label,
      etiqueta: nombre,
  ...
  SIGLOS.forEach((siglo, i) => {
    ...
    if (valor > 0) {
      item.addEventListener('click', () =>
        irATabla({ ...filtroBase, fecha: siglo, escala: 'siglo' }),
      );
    }
...
function crearFolderTabs({
  ...
  if (mencion) {
    ...
    tabMencion.addEventListener('click', () =>
      alternar('mencion', () => {
        const contenido = document.createElement('div');
        contenido.className = 'conteo-detalle-tab-contenido';
        contenido.innerHTML = `
  ...
  function renderDetalle(item: any) {
    ...
    ATRIBUTOS_DETALLE.forEach(({ nombre, etiqueta: etiquetaAtributo, valor }) => {
      ...
      for (let c = 0; c < CELDAS_WAFFLE; c++) {
        ...
        if (c >= CELDAS_WAFFLE - pct)
          celda.classList.add('conteo-detalle-atributo-panel-celda--llena');
