...
export function dibujarButterflyGenero({
  filtrosContainerId,
  chartContainerId,
  datos,
  crimenTop,
  ...
  function dibujarMariposa() {
    ...
    const filasGeneros = filas.filter(
      (d) => d.genero === estadoLocal.generoIzquierda || d.genero === estadoLocal.generoDerecha,
    );
    ...
    const filasConDatos = nombresFila.filter(
      (f: any) => conteoIzq.get(f)! > 0 || conteoDer.get(f)! > 0,
    );
    const IH = filasConDatos.length * PASO_FILA;
    const HEIGHT = MARGIN.top + IH + MARGIN.bottom;

    const máximoFila =
      d3.max(filasConDatos, (f: any) => Math.max(conteoIzq.get(f)!, conteoDer.get(f)!)) || 1;
    ...
    filasConDatos.forEach((nombreFila: string) => {
      ...
      function dibujarLado(valor: number, genero: string, color: string, esDerecha: boolean) {
        ...
        if (d) {
          const barra = filaGrupo
            .append('path')
            .attr('d', d)
            .attr('fill', color)
            .attr('class', 'mariposa-barra-dato');
          if (enSubnivel && valor > 0) {
            barra.style('cursor', 'pointer').on('click', (event: MouseEvent) => {
              event.stopPropagation();
              botonVerCasos.mostrar(`${genero} · ${nombreFila}`, () =>
                irATablasFiltradas({
                  genero,
    ...
    const etiquetaNivel = enSubnivel
      ? `Subcrímenes de "${crimenActivoLocal}"`
      : 'Todos los crímenes';
    nota.textContent =
      `${etiquetaNivel} · ${estadoLocal.generoIzquierda}: ${totalIzq.toLocaleString('es')} · ${estadoLocal.generoDerecha}: ${totalDer.toLocaleString('es')}` +
