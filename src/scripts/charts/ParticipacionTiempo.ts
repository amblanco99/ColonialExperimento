...
function contarPorDecada(filas: FilaCsv[], decadas: number[], generos: string[]) {
  const idsPorGeneroDecada = new Map<string, Map<number, Set<string>>>(
    generos.map(
      (g: string) =>
        [g, new Map(decadas.map((dc: number) => [dc, new Set<string>()]))] as [
          string,
...
export function dibujarParticipacionTiempo({
  chartContainerId,
  datos,
  decadas,
  generosActivos,
  ...
  function dibujarLinea(padre: HTMLElement, conteosPorGenero: any) {
    ...
    const maxValor =
      d3.max(generosActivos, (g: string) =>
        d3.max(conteosPorGenero.get(g), (d: FilaCsv) => d.valor),
      ) || 1;
    ...
    function seleccionarPunto(genero: string, década: number) {
      ...
      aplicarResaltadoLinea();
      botonVerCasos.mostrar(`${genero} · ${década}`, () =>
        irATablasFiltradas({
          genero,
