...
function contarPorDecada(eventos: FilaCsv[], decadas: number[], tipos: string[]) {
  const idsPorTipoDecada = new Map<string, Map<number, Set<string>>>(
    tipos.map(
      (t: string) =>
        [t, new Map(decadas.map((dc: number) => [dc, new Set<string>()]))] as [
          string,
...
export function dibujarParticipacionTiempoAgentes({
  chartContainerId,
  eventos,
  decadas,
  tiposActivos,
  crimenActivo,
}: OpcionesParticipacionAgentes) {
  ...
  function dibujarLinea(padre: HTMLElement, conteosPorTipo: any) {
    ...
    const maxValor =
      d3.max(tiposActivos, (t: string) => d3.max(conteosPorTipo.get(t), (d: FilaCsv) => d.valor)) ||
      1;

    const x = d3
      .scaleLinear()
      .domain(d3.extent(decadas) as [number, number])
      .range([0, IW]);
    const y = d3
      .scaleLinear()
      .domain([0, maxValor as number])
      .nice()
      .range([IH, 0]);
    ...
    function seleccionarPunto(tipo: string, década: number) {
      ...
      seleccionTipo = tipo;
      aplicarResaltadoLinea();
      botonVerCasos.mostrar(`${tipo} · ${década}`, () =>
        irATablasFiltradas({
          agente: tipo,
    ...
    const g = svg
      .append('g')
      .attr('transform', `translate(${MARGIN_LINEA.left},${MARGIN_LINEA.top})`);
