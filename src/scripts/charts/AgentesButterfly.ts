...
export function crearAgentesButterfly(containerId: string) {
  ...
  function actualizar(eventosFiltrados: FilaCsv[], crimenTop = 'Todos') {
    ...
    if (enSubnivel) {
      ...
      nombresFila = [
        ...d3.rollup(
          filasBase.filter((d) => d.subcrimen),
          (v) => v.length,
    ...
    const filasConDatos = nombresFila.filter(
      (f: any) => conteoInst.get(f)! > 0 || conteoPobl.get(f)! > 0,
    );
    const IH = filasConDatos.length * PASO_FILA;
    const HEIGHT = MARGIN.top + IH + MARGIN.bottom;
    const máximoFila =
      d3.max(filasConDatos, (f: any) => Math.max(conteoInst.get(f)!, conteoPobl.get(f)!)) || 1;
        ...
        if (d) {
          ...
          if (enSubnivel && valor > 0) {
            barra.style('cursor', 'pointer').on('click', (event: MouseEvent) => {
              ...
              botonVerCasos.mostrar(`${grupo} · ${nombreFila}`, () =>
                irATablasFiltradas({
    ...
    nota.textContent =
      `${etiquetaNivel} · Instituciones: ${totalInst.toLocaleString('es')} · Poblaciones: ${totalPobl.toLocaleString('es')}` +
